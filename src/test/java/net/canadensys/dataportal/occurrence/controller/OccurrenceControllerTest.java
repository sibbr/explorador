 ...
 * @Test public void test404HttpErrorPage() throws Exception {
 *       mockMvc.perform(get("/not_found")) .andExpect(status().isNotFound())
...
public class OccurrenceControllerTest extends
	...
	public void testIptResourceURL() throws Exception {
		...
		assertTrue(((RedirectView) mav.getView()).getUrl().contains(
				"search?iptresource=acad-specimens"));
	}
	...
	public void testOccurrenceURL() throws Exception {
		...
		HashMap<String, Object> modelRoot = (HashMap<String, Object>) mav
				.getModel().get(OccurrencePortalConfig.PAGE_ROOT_MODEL_KEY);
		OccurrenceModel occModel = (OccurrenceModel) modelRoot.get("occModel");
		assertEquals("TRT.6", occModel.getDwcaid());
	}

	/**
	 * Test the model used for display purpose. associatedSequences urls format
	 * are defined in src/main/resources/references/sequenceProviders.properties
	 */
	@Test
	public void testOccurrenceViewModel() {
		...
		OccurrenceViewModel occViewModel = occurrenceController
				.buildOccurrenceViewModel(occModel, null, null, Locale.ENGLISH);

		// Map<String,List<Pair<String,String>>>
		// associatedSequencesPerProviderMap =
		// occViewModel.getAssociatedSequencesPerProviderMap();
		List<String> associatedSequences = occViewModel
				.getAssociatedSequences();
		assertEquals(4, associatedSequences.size());

		assertEquals(
				"http://bins.boldsystems.org/index.php/Public_BarcodeCluster?clusteruri=BOLD:AAJ7963",
				associatedSequences.get(0));
		assertEquals(
				"http://www.boldsystems.org/connectivity/specimenlookup.php?processid=1234",
				associatedSequences.get(1));
		assertEquals(
				"http://www.boldsystems.org/connectivity/specimenlookup.php?processid=2345",
				associatedSequences.get(2));
