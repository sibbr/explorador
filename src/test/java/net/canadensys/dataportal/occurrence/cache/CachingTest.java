...
public class CachingTest extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void testDistinctValuesCountCache() {
		...
		Integer count = searchService.getDistinctValuesCount(searchCriteria,
				(OccurrenceSearchableField) countrySearchableField);
		assertTrue(count.intValue() > 0);

		// extract value from cache
		Cache cache = CacheManager.getCacheManager(CacheManager.DEFAULT_NAME)
				.getCache("distinctValuesCountCache");
		Integer countFromCache = (Integer) cache.get(
				countrySearchableField.getSearchableFieldId()).getObjectValue();
