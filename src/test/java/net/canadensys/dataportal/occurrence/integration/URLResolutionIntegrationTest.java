...
/**
 * Test URL resolving only, this class only look at response code and won't
 * render the page.
 * 
 * @author cgendreau
 * 
...
public class URLResolutionIntegrationTest {
	...
	public void testURLThatShouldNotResolve() {
		...
		assertResponseCode(AbstractIntegrationTest.TESTING_SERVER_URL_LEGACY
				+ "toto", 404);
		// wrong URL with language specified
		assertResponseCode(AbstractIntegrationTest.TESTING_SERVER_URL + "toto",
				404);
		// conflicted language URL
		assertResponseCode(AbstractIntegrationTest.TESTING_SERVER_URL_LEGACY
				+ "fr/search?view=table", 404);
	}

	@Test
	public void testResolveLegacyURL() {
		assertResponseCode(AbstractIntegrationTest.TESTING_SERVER_URL_LEGACY
				+ "r/acad-specimens", 200);
