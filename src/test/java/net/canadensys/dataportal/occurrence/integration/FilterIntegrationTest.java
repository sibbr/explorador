...
public class FilterIntegrationTest extends AbstractIntegrationTest {
	...
	public void selectAndAddFilter() {
		...
		List<WebElement> suggestionList = filterContent.findElements(By
				.cssSelector("table#value_suggestions tbody tr td"));
		// TODO add wait
		suggestionList.get(0).click();

		// validate current filter
		assertTrue(isCurrentFilterCategory(
				browser.findElement(By.id("search")), "Country"));
	...
	/**
	 * Get a WebElement list representing the current filter under a specific
	 * parent.
	 * 
	 * @param parent
	 *            must contains the <form> element
	 * @return
	 */
	public static List<WebElement> getCurrentFilterElements(WebElement parent) {
		...
		return formContent.findElements(By
				.cssSelector("ul#filter_current li.filter"));
	}

	/**
	 * Check if a filter category is present under a specific parent. xpath is
	 * used since getText() will also returns the sub-elements and
	 * partialLinkText() may give false positive.
	 * 
	 * @param parent
	 *            must contains the <form> element
	 * @param filterCategory
	 *            a filter category is like Country, Kingdom
	 * @return
	 */
	public static boolean isCurrentFilterCategory(WebElement parent,
			String filterCategory) {
		WebElement formContent = parent.findElement(By.tagName("form"));
		try {
			formContent.findElement(By.xpath("ul/li[text() = '"
					+ filterCategory + "']"));
