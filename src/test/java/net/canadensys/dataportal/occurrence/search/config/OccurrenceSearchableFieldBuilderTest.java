...
/**
 * This test ensures OccurrenceSearchableFieldBuilder can actually build a
...
public class OccurrenceSearchableFieldBuilderTest {
	...
	public void testOccurrenceSearchableFieldBuilder() {
		OccurrenceSearchableFieldBuilder b = new OccurrenceSearchableFieldBuilder(
				1, "test-field");
		// make sure we never return an incomplete OccurrenceSearchableField
		assertNull(b.toOccurrenceSearchableField());

		// test method chaining
		OccurrenceSearchableField occField = b.eqOperator()
				.singleValue("testField", String.class)
				.toOccurrenceSearchableField();

		assertEquals(QueryOperatorEnum.EQ,
				occField.getSupportedOperator().get(0));
		...
		assertTrue(occField.getSupportedOperator().contains(
				QueryOperatorEnum.ELIKE));
	}
}
