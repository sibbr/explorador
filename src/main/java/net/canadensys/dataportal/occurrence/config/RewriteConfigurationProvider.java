...
/**
 * URL Rewriting configuration. supportedLocale are loaded from configuration
 * file defined in OccurrencePortalConfig.
 * 
 * @author cgendreau
 * 
 */
...
public class RewriteConfigurationProvider extends HttpConfigurationProvider {
	...
	public Configuration getConfiguration(final ServletContext context) {
		...
		return ConfigurationBuilder
				...
				// handle root path
				.addRule()
				.when(Path.matches("/").and(Direction.isInbound())
				...
				.perform(new RedirectHomePage())
				.addRule()
				.when(Path.matches("/{tail}").and(Direction.isInbound())
						.andNot(DispatchType.isForward()))
				.perform(new HandleLegacyURL())
				.where("tail")
				.matches("(?!(" + languageRegexPart + "|assets|ws)).*")
				.addRule()
				.when(Path.matches("/{lang}/{path}").and(Direction.isInbound()))
				.perform(Join.path("/{lang}/{path}").to("/{path}"))
				.where("lang")
				.matches(languageRegexPart)
				.where("path")
				.configuredBy(
						LocaleTransposition.bundle(
								OccurrencePortalConfig.URL_BUNDLE_NAME, "lang"))
				.addRule()
				.when(Path.matches("/{lang}/{path1}/{id1}").and(
						Direction.isInbound()))
				.perform(
						Join.path("/{lang}/{path1}/{id1}").to("/{path1}/{id1}"))
				.where("lang")
				.matches(languageRegexPart)
				.where("path1")
				.configuredBy(
						LocaleTransposition.bundle(
								OccurrencePortalConfig.URL_BUNDLE_NAME, "lang"))
				.addRule()
				.when(Path.matches("/{lang}/{path1}/{id1}/{path2}").and(
						Direction.isInbound()))
				.perform(
						Join.path("/{lang}/{path1}/{id1}").to(
								"/{path1}/{id1}/{path2}"))
				.where("lang")
				.matches(languageRegexPart)
				.where("path1")
				.configuredBy(
						LocaleTransposition.bundle(
								OccurrencePortalConfig.URL_BUNDLE_NAME, "lang"))
				.addRule()
				.when(Path.matches("/{lang}/{path1}/{id1}/{path2}/{id2}").and(
						Direction.isInbound()))
				.perform(
						Join.path("/{lang}/{path1}/{id1}/{path2}/{id2}").to(
								"/{path1}/{id1}/{path2}/{id2}"))
				.where("lang")
				.matches(languageRegexPart)
				.where("path1")
	...
	/**
	 * Load supported languages from configuration file. Languages must be comma
	 * separated. This class is loaded before Spring web application so it is
	 * more reliable to read the configuration file ourselves.
	 * 
	 * @param context
	 */
	private void loadSupportedLocale(final ServletContext context) {
		final InputStream is = context
				.getResourceAsStream(OccurrencePortalConfig.CONFIG_FILENAME);

		Properties configProperties = new Properties();
		try {
			configProperties.load(is);
			defaultLanguage = StringUtils.defaultString(configProperties
					.getProperty(OccurrencePortalConfig.DEFAULT_LANGUAGE_KEY),
	...
	private class RedirectHomePage extends HttpOperation {
		public void performHttp(HttpServletRewrite event,
			...
			Redirect.permanent(event.getContextPath() + landingUrl).perform(
					event, context);
		}
	}

	/**
	 * Custom HttpOperation to handle legacy urls that were used in previous
	 * version of the Explorer. Those urls may have been bookmarked of
	 * referenced somewhere so we want still want to resolve them.
	 * 
	 * @author cgendreau
	 * 
	 */
	private class HandleLegacyURL extends HttpOperation {
		public void performHttp(HttpServletRewrite event,
			...
			if (StringUtils.isNotBlank(event.getRequest().getParameter(
					LANG_PARAM))) {
				if (supportedLocale.contains(event.getRequest().getParameter(
						LANG_PARAM))) {
			...
			if (urlParts.length >= 1) {
				...
				if (urlParts[0].equalsIgnoreCase("search")) {
					landingUrl = I18nUrlBuilder.generateI18nResourcePath(lang,
							OccurrencePortalConfig.I18N_TRANSLATION_HANDLER
									.getTranslationFormat("search"),
							(String) null);

					// if query string was provided, keep it if necessary
					if (StringUtils.isNotBlank(event.getRequest()
							.getQueryString())) {
						String queryString = getCleanedQueryString(event
								.getRequest());
				...
				if (urlParts[0].equalsIgnoreCase("r")
						|| urlParts[0].equalsIgnoreCase("d")) {
					if ((urlParts.length == 2)
							&& StringUtils.isNotBlank(urlParts[1])) {
						landingUrl = I18nUrlBuilder.generateI18nResourcePath(
								lang,
								OccurrencePortalConfig.I18N_TRANSLATION_HANDLER
										.getTranslationFormat("resource"),
								urlParts[1]);
					}
				}
			}
		...
		/**
		 * Return query string without unnecessary elements. e.g. lang parameter
		 * is not used anymore.
		 * 
		 * @param request
		 * @return
		...
		private String getCleanedQueryString(HttpServletRequest request) {
			UriComponentsBuilder bldr = ServletUriComponentsBuilder
					.fromRequest(request);
