...
public class RewriteConfigurationProvider extends HttpConfigurationProvider {
	...
	public Configuration getConfiguration(final ServletContext context) {
		...
		return ConfigurationBuilder
				...
				.addRule().when(Path.matches("").and(Direction.isInbound()).andNot(DispatchType.isForward())).perform(new RedirectHomePage())
				.addRule().when(Path.matches("/{tail}").and(Direction.isInbound()).andNot(DispatchType.isForward())).perform(new HandleLegacyURL())
				.where("tail").matches("(?!(" + languageRegexPart + "|assets|ws)).*").addRule()
				.when(Path.matches("/{lang}/{path}").and(Direction.isInbound())).perform(Join.path("/{lang}/{path}").to("/{path}")).where("lang")
				.matches(languageRegexPart).where("path").configuredBy(LocaleTransposition.bundle(OccurrencePortalConfig.URL_BUNDLE_NAME, "lang"))
				.addRule().when(Path.matches("/{lang}/{path1}/{id1}").and(Direction.isInbound()))
				.perform(Join.path("/{lang}/{path1}/{id1}").to("/{path1}/{id1}")).where("lang").matches(languageRegexPart).where("path1")
				.configuredBy(LocaleTransposition.bundle(OccurrencePortalConfig.URL_BUNDLE_NAME, "lang")).addRule()
				.when(Path.matches("/{lang}/{path1}/{id1}/{path2}").and(Direction.isInbound()))
				.perform(Join.path("/{lang}/{path1}/{id1}").to("/{path1}/{id1}/{path2}")).where("lang").matches(languageRegexPart).where("path1")
				.configuredBy(LocaleTransposition.bundle(OccurrencePortalConfig.URL_BUNDLE_NAME, "lang")).addRule()
				.when(Path.matches("/{lang}/{path1}/{id1}/{path2}/{id2}").and(Direction.isInbound()))
				.perform(Join.path("/{lang}/{path1}/{id1}/{path2}/{id2}").to("/{path1}/{id1}/{path2}/{id2}")).where("lang")
				.matches(languageRegexPart).where("path1").configuredBy(LocaleTransposition.bundle(OccurrencePortalConfig.URL_BUNDLE_NAME, "lang"))
	...
	private void loadSupportedLocale(final ServletContext context) {
		...
		try {
			...
			defaultLanguage = StringUtils.defaultString(configProperties.getProperty(OccurrencePortalConfig.DEFAULT_LANGUAGE_KEY),
	...
	private class RedirectHomePage extends HttpOperation {
		public void performHttp(HttpServletRewrite event, EvaluationContext context) {
			...
			String landingUrl = I18nUrlBuilder.generateI18nResourcePath(reqLanguage,
	...
	private class HandleLegacyURL extends HttpOperation {
		public void performHttp(HttpServletRewrite event, EvaluationContext context) {
			...
			if (urlParts.length >= 1) {
				...
				if (urlParts[0].equalsIgnoreCase("r") || urlParts[0].equalsIgnoreCase("d")) {
					if ((urlParts.length == 2) && StringUtils.isNotBlank(urlParts[1])) {
						landingUrl = I18nUrlBuilder.generateI18nResourcePath(lang,
								OccurrencePortalConfig.I18N_TRANSLATION_HANDLER.getTranslationFormat("resource"), urlParts[1]);
					}
				}
			}
			...
			if (landingUrl == null) {
				landingUrl = I18nUrlBuilder.generateI18nResourcePath(lang,
						OccurrencePortalConfig.I18N_TRANSLATION_HANDLER.getTranslationFormat("404"));
			}
