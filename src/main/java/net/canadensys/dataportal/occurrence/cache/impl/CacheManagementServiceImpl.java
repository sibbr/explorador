...
public class CacheManagementServiceImpl implements CacheManagementServiceIF {
	...
	static {
		...
		MANAGED_CACHE_KEY
				.add(CacheManagementServiceIF.RESOURCE_MODEL_CACHE_KEY);
	}
	...
	/**
	 * Blocking function, caller is responsible to run this in a thread if
	 * needed.
	 */
	@Override
	public void preLoadCache() {
		...
		for (SearchableFieldEnum currSf : SearchableFieldGroupEnum.CLASSIFICATION
				.getContent()) {
			osf = searchServiceConfig.getSearchableField(currSf);
			occurrenceSearchService.getDistinctValuesCount(emptySearchCriteria,
					osf);
		}
		for (SearchableFieldEnum currSf : SearchableFieldGroupEnum.LOCATION
				.getContent()) {
			osf = searchServiceConfig.getSearchableField(currSf);
			occurrenceSearchService.getDistinctValuesCount(emptySearchCriteria,
					osf);
	...
	public void purgeCache() {
		...
		for (String currCacheKey : MANAGED_CACHE_KEY) {
			cache = CacheManager.getCacheManager(CacheManager.DEFAULT_NAME)
					.getCache(currCacheKey);
			if (cache != null) {
				cache.removeAll();
			}
		}
