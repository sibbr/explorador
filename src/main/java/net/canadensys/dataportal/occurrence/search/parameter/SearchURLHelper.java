...
/**
 * This class helps dealing with some parameters in the URL of the search
 * service.
 * 
 * @author canadensys
 * 
...
public class SearchURLHelper {
	...
	public enum ViewNameEnum {
		TABLE_VIEW_NAME("table"), MAP_VIEW_NAME("map"), IMAGE_VIEW_NAME("image"), STATS_VIEW_NAME(
				"stats");
	...
	/**
	 * This method will create a search table view URL based on baseUrl and
	 * queryParams. Only the search query parameters will be kept and the
	 * view=table will be added Useful to keep the search query string among
	 * different pages
	 * 
	 * @param baseUrl
	 * @param queryParams
	 * @return
	 */
	public static String createSearchTableViewURL(String baseUrl,
			Map<String, String[]> queryParams) {
		UriComponentsBuilder compBuilder = ServletUriComponentsBuilder
				.fromPath(baseUrl + SEARCH_CONTEXT);
		...
		while (keyIt.hasNext()) {
			...
			if (SearchParamParser.isSearchParam(currKey)) {
				compBuilder.queryParam(currKey,
						(Object[]) queryParams.get(currKey));
			}
		}
		compBuilder.queryParam(VIEW_PARAM,
				ViewNameEnum.TABLE_VIEW_NAME.getViewName());
