...
/**
 * This class is responsible to create a SearchSortPart based on URL parameters.
 * This class is designed for Occurrence Portal search.
 * 
 * @author canadensys
 * 
 */
public class SearchSortPartParser {
	...
	/**
	 * Create a SearchSortPart from query parameters if they contain at least
	 * one SearchSortPart related parameter.
	 * 
	 * @param parametersMap
	 * @return SearchSortPart instance or null is no SearchSortPart related
	...
	public SearchSortPart parse(Map<String, String[]> parametersMap) {
		...
		if (searchConfig == null) {
			throw new IllegalArgumentException(
					"SearchServiceConfig must not be null.");
		}
		...
		for (String currParameterKey : parametersMap.keySet()) {
			...
			if (PARAM_PAGE.equalsIgnoreCase(currParameterKey)) {
				...
				searchSortPart.setPageNumber(NumberUtils.toInt(
						currentParameterValue, 1));
			} else if (PARAM_SORT_BY.equalsIgnoreCase(currParameterKey)) {
				// ensure it's a know field
				SearchableField searchableField = searchConfig
						.getSearchableFieldbyName(currentParameterValue);
				// only support sort for 'SINGLE_VALUE' otherwise it's kind of
				// hard to define
				if (searchableField != null
						&& searchableField.getSearchableFieldTypeEnum() == SearchableFieldTypeEnum.SINGLE_VALUE) {
					if (searchSortPart == null) {
						searchSortPart = new SearchSortPart();
					}
					searchSortPart.setOrderByColumn(searchableField
							.getRelatedField());
				}
			} else if (PARAM_SORT.equalsIgnoreCase(currParameterKey)) {
				if (EnumUtils.isValidEnum(OrderEnum.class,
						currentParameterValue.toUpperCase())) {
					if (searchSortPart == null) {
						searchSortPart = new SearchSortPart();
					}
					searchSortPart.setOrder(OrderEnum
							.valueOf(currentParameterValue.toUpperCase()));
				}
			}
		}
