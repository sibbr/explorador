...
public class OccurrenceSearchServiceImpl implements OccurrenceSearchService {
	...
	private static final Logger LOGGER = Logger
			.getLogger(OccurrenceSearchServiceImpl.class);
	...
	public LimitedResult<List<Map<String, String>>> searchWithLimit(
		...
		return occurrenceDAO.searchWithLimit(searchCriteria,
				SearchServiceConfig.OCCURRENCE_SEARCH_FIELDS);
	}
	...
	public LimitedResult<List<Map<String, String>>> searchWithLimit(
		...
		return occurrenceDAO.searchWithLimit(searchCriteria,
				SearchServiceConfig.OCCURRENCE_SEARCH_FIELDS, searchSortPart);
	}

	@Override
	@Transactional(readOnly = true)
	public OccurrenceModel getOccurrenceSummary(int auto_id) {
		return occurrenceDAO.loadOccurrenceSummary(auto_id,
				SearchServiceConfig.OCCURENCE_SUMMARY_FIELDS);
	}
	...
	public SearchQueryPart getSearchQueryPartFromFieldName(String fieldName,
			String fieldValue) {
		OccurrenceSearchableField searchField = (OccurrenceSearchableField) searchServiceConfig
		...
		sqp.addParsedValue(fieldValue, searchField.getRelatedField(),
				fieldValue);
		return sqp;
	}
	...
	public DownloadResultStatus searchAsDwca(
		...
		String email = (String) extraProperties
				.get(DownloadPropertiesEnum.EMAIL);
	...
	/**
	 * This method will generate the DarwinCore archive and send an email
	 * containing the link to download it.
	 */
	@Override
	public void getResultAsDwcaAsync(
			final Map<String, List<SearchQueryPart>> searchCriteria,
			final String fileName,
			final Map<DownloadPropertiesEnum, Object> extraProperties) {
		...
		taskExecutor.execute(new Runnable() {
			...
			public void run() {
				...
				String fullPath = FilenameUtils.concat(
						searchServiceConfig.getGeneratedContentFolder(),
						fileName);
				String fullFilePath = fullPath + ZipUtils.ZIP_EXT;
				String emailAddress = (String) extraProperties
						.get(DownloadPropertiesEnum.EMAIL);

				String md5emailAddress = emailAddress;
				// should we hash the email address?
				if (appConfig.isHashEmailAddress()) {
					md5emailAddress = DigestUtils.md5Hex(emailAddress
							+ StringUtils.defaultString(appConfig
									.getEmailSalt()));
				}

				// log the download model prior the dwca generation
				DownloadLogModel downloadLogModel = dwcaBuilder.logDownload(
						md5emailAddress, searchCriteria.toString());

				File dwcaFile = dwcaBuilder.generatesDarwinCoreArchive(
						DWCA_HEADERS, searchCriteria, fullPath, fullFilePath);
				if (dwcaFile != null && dwcaFile.exists()) {
					// send email
					String fileURL = StringUtils.appendIfMissing(
							searchServiceConfig.getPublicDownloadURL(), "/")
							+ dwcaFile.getName();

					Locale locale = (Locale) extraProperties
							.get(DownloadPropertiesEnum.LOCALE);
					...
					templateData.put(
							"institutionCodeList",
							StringEscapeUtils.escapeHtml4(StringUtils.join(
									dwcaBuilder
											.getDistinctInstitutionCode(searchCriteria),
									", ")));
					templateData.put("requestTimestamp", now);
					templateData.put("requestTimestampText",
							new SimpleDateFormat(EMAIL_DATE_FORMAT, locale)
									.format(now));
					templateData.put("requestURL", extraProperties
							.get(DownloadPropertiesEnum.SEARCH_URL));

					String templateName = appConfig
							.getDownloadEmailTemplateName(locale);
					if (!mailSender.sendMessage(emailAddress,
							bundle.getString("download.dwca.email.subject"),
							templateData, templateName)) {
						LOGGER.fatal("Supposed to send DarwinCore archive by email to "
								+ md5emailAddress + " but it failed.");
					} else {
						int occurrenceCount = dwcaBuilder
	 ...
	 *            StatsPropertiesEnum.RESOURCE_BUNDLE,
	 *            StatsPropertiesEnum.MAX_RESULT
	 * @return
	 */
	@Override
	@Transactional(readOnly = true)
	public ChartModel getValuesFrequencyDistribution(
		...
		ResourceBundle resourceBundle = (ResourceBundle) extraProperties
				.get(StatsPropertiesEnum.RESOURCE_BUNDLE);
		Integer maxResult = (Integer) extraProperties
				.get(StatsPropertiesEnum.MAX_RESULT);
		...
		List<SimpleImmutableEntry<String, Integer>> results = occurrenceDAO
				.getValueCount(searchCriteria, column.getRelatedField(),
						maxResult);
		...
		for (SimpleImmutableEntry<String, Integer> currRow : results) {
			...
			if (!StringUtils.isBlank(currRow.getKey())) {
				if (column.getType().equals(String.class)) {
					...
					values[0] = NumberUtils.parseNumber(currRow.getKey(),
							column.getType());
					if (values[0] == null) {
		...
		if (currCount != total) {
			Object[] values = new Object[2];
			// if the number of row is lower than maxResult, it means that the
			// remaining rows are null or empty.
			if (maxResult != null && cm.getRowCount() < maxResult) {
				values[0] = resourceBundle.getString("view.stats.novalue");
			} else {// here, the remaining rows contain other value including
					// null or empty value
		...
		for (SimpleImmutableEntry<String, Integer> currRow : results) {
			...
			if (!StringUtils.isBlank(currRow.getKey())) {
				if (column.getType().equals(String.class)) {
					...
					key = NumberUtils.parseNumber(currRow.getKey(),
							column.getType());
					if (key == null) {
						LOGGER.fatal("Unparsable column type");
					}
	...
	/**
	 * Get the number of distinct values for a specific column. If the column is
	 * String based(column.getType()), NULL and empty rows are excluded from the
	 * count. Caching is used ONLY when searchCriteria.isEmpty().
	 * 
	 * @param searchCriteria
	 * @param column
	 */
	...
	public Integer getDistinctValuesCount(
			Map<String, List<SearchQueryPart>> searchCriteria,
			OccurrenceSearchableField column) {
		// copy(deep copy not needed) searchCriteria to not add the non-empty
		// criteria in the provided object.
		...
		return occurrenceDAO.getCountDistinct(innerSearchCriteria,
				column.getRelatedField());
	}
	 ...
	 * @param column
	 * @param searchCriteria
	 *            will be modified if 'column' is not already in the
	 *            searchCriteria
	 */
	private void addNonEmptyCriteria(OccurrenceSearchableField column,
			Map<String, List<SearchQueryPart>> searchCriteria) {
		if (column.getType() == String.class
				&& column.getSearchableFieldTypeEnum() == SearchableFieldTypeEnum.SINGLE_VALUE
