...
/**
 * This class is an utility class to allow creation of a DarwinCore archive in a
 * separate class. This class resides at the Service layer. The main goal is to
 * allow the method to run asynchronously with its own transaction.
 * 
 * @author canadensys
 * 
 */
...
public class DwcaBuilder {
	...
	public File generatesDarwinCoreArchive(List<String> headers,
			Map<String, List<SearchQueryPart>> searchCriteria, String fullPath,
			String fullFilePath) {
		...
		if (OccurrenceDwcWriter.write(headers, dwcaFolder, it)) {
			if (ZipUtils.zipFolder(dwcaFolder, dwcaFile.getAbsolutePath())) {
				try {
					...
					LOGGER.fatal("Failed to delete Dwca working dir at "
							+ dwcaFolder, e);
				}
			} else {
	...
	@Transactional(readOnly = true)
	public List<String> getDistinctInstitutionCode(
			Map<String, List<SearchQueryPart>> searchCriteria) {
		List<String> result = occurrenceDAO
