...
/**
 * This function is the entry point to safely get a collection of
 * SearchQueryPart based on URL parameters. Thread safe after configuration
 * 
 * @author canadensys
 * 
 */
...
public class SearchParamHandler {
	...
	private static final Logger LOGGER = Logger
			.getLogger(SearchParamHandler.class);
	...
	/**
	 * This functions will return a Collection of valid SearchQueryPart
	 * (according to SearchQueryPartValidator).
	 * 
	 * @param parametersMap
	 * @return
	 */
	public Collection<SearchQueryPart> getSearchQueryPartCollection(
			Map<String, String[]> parametersMap) {
		Collection<SearchQueryPart> searchRelatedParams = paramParser
				.parse(parametersMap);
		Iterator<SearchQueryPart> searchParamsIt = searchRelatedParams
				.iterator();
	 ...
	 * 
	 * @param parametersMap
	 * @return empty map or map containing the parameters(and the value) related
	 *         to a search query.
	 */
	public Map<String, String> getSearchQueryRelatedParameters(
			Map<String, String[]> parametersMap) {
		Map<String, String> queryParameters = new HashMap<String, String>();
		for (String currParamKey : parametersMap.keySet()) {
			if (SearchParamParser.isSearchParam(currParamKey)) {
				queryParameters.put(currParamKey,
						parametersMap.get(currParamKey)[0]);
			}
		}
	...
	/**
	 * Returns the queryPartColl as list of SearchQueryPart mapped by field
	 * name. If your work with the fieldId, you can provide a replacement map.
	 * This will allow the function to provide the matching name in the
	 * SearchQueryPart objects. String.
	 * 
	 * @param queryPartColl
	 *            validated result of SearchParamParser.parse(...)
	 * @param fieldIdNameMap
	 *            optional if the parametersMap contains the field name and not
	 *            the field id
	 * @return
	 */
	public Map<String, List<SearchQueryPart>> asMap(
			Collection<SearchQueryPart> queryPartColl) {
	...
	/**
	 * Returns the queryPartColl as list of SearchQueryPart. If your work with
	 * the fieldId, you can provide a replacement map. This will allow the
	 * function to provide the matching name in the SearchQueryPart objects.
	 * String.
	 * 
	 * @param queryPartColl
	 *            validated result of SearchParamParser.parse(...)
	 ...
	 * @return
	 */
	public List<SearchQueryPart> asList(
			Collection<SearchQueryPart> queryPartColl) {
	 ...
	 * @param sqpList
	 * @param targetType
	 * @return list of SearchQueryPart matching targetType or empty list if
	 *         none. Never null
	 */
	public List<SearchQueryPart> findSearchQueryPartByType(
			List<SearchQueryPart> sqpList, SearchableFieldTypeEnum targetType) {
