...
public class SearchParamParser {
	...
	protected void parseSingleValue(SearchQueryPart currSearchQueryPart) {
		if (String.class.equals(currSearchQueryPart.getSearchableField().getType())) {
			currSearchQueryPart.addParsedValue(currSearchQueryPart.getSingleValue(), currSearchQueryPart.getSingleField(),
					currSearchQueryPart.getSingleValue());
		}
		else if (Number.class.isAssignableFrom(currSearchQueryPart.getSearchableField().getType())) {
			currSearchQueryPart.addParsedValue(currSearchQueryPart.getSingleValue(), currSearchQueryPart.getSingleField(),
					NumberUtils.parseNumber(currSearchQueryPart.getSingleValue(), currSearchQueryPart.getSearchableField().getType()));
		}
	...
	protected void parseMinMaxNumber(SearchQueryPart currSearchQueryPart) {
		...
		if (valueList.size() == 1) {
			currSearchQueryPart.addParsedValue(valueList.get(0), fieldList.get(0),
					NumberUtils.parseNumber(valueList.get(0), currSearchQueryPart.getSearchableField().getType()));
			currSearchQueryPart.addParsedValue(valueList.get(0), fieldList.get(1),
					NumberUtils.parseNumber(valueList.get(0), currSearchQueryPart.getSearchableField().getType()));
		}
		else if (valueList.size() == 2) {
			currSearchQueryPart.addParsedValue(valueList.get(0), fieldList.get(0),
					NumberUtils.parseNumber(valueList.get(0), currSearchQueryPart.getSearchableField().getType()));
			currSearchQueryPart.addParsedValue(valueList.get(1), fieldList.get(1),
					NumberUtils.parseNumber(valueList.get(1), currSearchQueryPart.getSearchableField().getType()));
		}
		else {
	...
	protected void parseStartEndDate(SearchQueryPart currSearchQueryPart) {
		...
		for (int i = 0; i < valueList.size(); i++) {
			...
			if (m.find()) {
				currSearchQueryPart
						.addParsedValue(value, SearchServiceConfig.START_YEAR_PROPERTY, NumberUtils.parseNumber(m.group(1), Integer.class)); // year
				currSearchQueryPart.addParsedValue(value, SearchServiceConfig.START_MONTH_PROPERTY,
	...
	protected void parseInsidePolygon(SearchQueryPart currSearchQueryPart) {
		...
		for (int i = 0; i < valueList.size(); i++) {
			...
			if (m.find()) {
				currSearchQueryPart.addParsedValue(value, relatedField, Pair.of(m.group(1), m.group(2))); // lat,long
			}
			else {// simply clear the value to make this query part invalid
				currSearchQueryPart.clearValues();
	...
	protected void parseWithinRadius(SearchQueryPart currSearchQueryPart) {
		...
		Matcher m = GEO_COORDINATES_PARAM_PATTERN.matcher(value);
		if (m.find()) {
			currSearchQueryPart.addParsedValue(value, currSearchQueryPart.getSearchableField().getRelatedField(), Pair.of(m.group(1), m.group(2))); // lat,long
		}
		else {
			parsed = false;
		}
		...
		try {
			currSearchQueryPart.addParsedValue(value, currSearchQueryPart.getSearchableField().getRelatedField(), Double.valueOf(value).intValue());
		}
