...
public class OccurrenceSearchableField implements SearchableField {
	...
	public boolean equals(Object obj) {
		...
		return new EqualsBuilder().append(searchableFieldId, osf.searchableFieldId).append(searchableFieldName, osf.searchableFieldName)
				.append(type, osf.type).append(searchableFieldTypeEnum, osf.searchableFieldTypeEnum).append(supportSuggestion, osf.supportSuggestion)
				.append(supportPartialMatch, osf.supportPartialMatch).append(supportSelectionList, osf.supportSelectionList)
				.append(supportedOperator, osf.supportedOperator).append(relatedFields, osf.relatedFields)
				.append(extraProperties, osf.extraProperties).isEquals();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder(17, 37).append(searchableFieldId).append(searchableFieldName).append(type).append(searchableFieldTypeEnum)
				.append(supportSuggestion).append(supportPartialMatch).append(supportSelectionList).append(supportedOperator).append(relatedFields)
				.append(extraProperties).toHashCode();
	}
