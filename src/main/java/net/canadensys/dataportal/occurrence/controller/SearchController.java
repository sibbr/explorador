...
public class SearchController {
	...
	/**
	 * Write the Java bean as JSON String. All exceptions are transfered to the
	 * logger.
	 * 
	 * @param value
	 *            Java bean
	...
	private void handleSearchStatsView(HashMap<String, Object> model, HttpServletRequest request, Map<String, List<SearchQueryPart>> searchCriteria) {
		...
		Integer count = 0;
		// Only computes count for the entire group when there is more than one
	 ...
	 * Main task is to set 'hint' if the query is crossing the IDL. Safe to call
	 * if searchCriteria contains no geospatial query.
	 * 
	 * @param searchCriteria
	 */
	private void handleGeospatialQuery(Map<String, List<SearchQueryPart>> searchCriteria) {
		...
		if (insidePolygonSqp != null && !insidePolygonSqp.isEmpty()) {
			// we could save, in most of the cases, a call to the db by checking
	 ...
	 *         {"extentMin":["-44","-40.0833333"
	 *         ],"extentMax":["39","103.4166667"
	 *         ],"centroid":["-2.5","31.6666667"]}
	 */
	...
	/**
	 * Handling the request for a download of the result of a query. For the
	 * moment, only DarwinCore archive format can be returned. The service will
	 * only return when the archive will be generated.
	 * 
	 * @param request
	 * @return
	 */
