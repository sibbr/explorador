...
public class EOLPage {
	...
	private List<EOLDataObject> dataObjects;

	/**
	 * Basic constructor for POJO
	 */
	public EOLPage(String id, String richnessScore, ArrayList<EOLSynonym> syonyms,
			ArrayList<EOLVernacularName> vernacularNames, ArrayList<EOLTaxonConcept> taxonConcepts, ArrayList<EOLDataObject> dataObjects) {
		setId(id);
	...
	public String toString() {
		...
		for (EOLTaxonConcept t: getTaxonConcepts())
			output += t.toString();
		output += "\n \t Data Objects: " + getDataObjects().size() + "\n";
		for (EOLDataObject d: getDataObjects())
			output += d.toString();
		return output;
	}
	 ...
	 */
	public void setDataObjects(List<EOLDataObject> dataObjects) {
