...
public class EOLImage extends EOLDataObject {
	 ...
	 * @param agents
	 */
	public EOLImage(String identifier, String dataObjectVersionID,
			...
			String description, String mediaURL, String eolThumbnail,
			String location, List<EOLAgent> agents) {
			...
			for (int i = 0; i < json.length(); i++) {
				...
				if (!item.isNull("mimeType")) {
					...
					if (mime.equalsIgnoreCase("image/jpeg")) {
						...
						images.add(new EOLImage(identifier, dataObjectVersionID, dataType,
								vettedStatus, dataRating, mimeType, title, language,
								license, rights, rightsHolder, source, description,
								mediaUrl, thumb, location, agents));
					}
				}
			}
