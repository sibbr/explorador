<div id="bhlcontent"> 
  ...
  <ul>
    <#if page.occBHL?has_content> <a href="http://www.biodiversitylibrary.org/name/${page.occModel.scientificname?if_exists}" target="_blank"> <i>${rc.getMessage("occpage.other.search.bhl")}</i> </a>
    <h3>${rc.getMessage("occpage.bhl.format")}: </h3>
    n. <a href="#" style="color: #36b"><i>${rc.getMessage("occpage.bhl.format.publication.title")}</i></a> ${rc.getMessage("occpage.bhl.format.publication.location")} 
    ...
    ${page_index + 1}. <a href="http://www.biodiversitylibrary.org/page/${page.getPageId()}" target="_blank" style="color: #36b"> <i>${page.getShortTitle()}</i> </a> <#if page.getPublisherPlace()?? && page.getPublisherPlace()?has_content>
    ${page.getPublisherPlace()}
    </#if>
    <#if page.getPublisherName()?has_content>
    ${page.getPublisherName()}
    <#else> <span class="remark">(${rc.getMessage("occpage.bhl.publisher.name.not.provided")})</span> </#if>	
    <#if page.getPublicationDate()?? && page.getPublicationDate()?has_content>
    ${page.getPublicationDate()}
    <#else> <span class="remark">(${rc.getMessage("occpage.bhl.publication.date.not.provided")})</span> </#if> </br>
    </#list>
    </#if>
    </#list>
    <#else>
